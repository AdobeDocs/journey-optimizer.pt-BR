---
title: Criar uma inserção de extensão
description: As estratégias de Exd consistem em coleções associadas a restrições e métodos de classificação para determinar ofertas.
...
# Criar uma inserção de extensão {#create-exd-placement}

Você pode criar uma inserção de extensão fazendo uma solicitação POST para a API da biblioteca de ofertas.
...
Uma resposta bem-sucedida retorna os detalhes do posicionamento exd recém-criado, incluindo a id. Você pode usar a ID em etapas posteriores para atualizar ou excluir a inserção de exd.
