...
Para obter a documentação completa da API, incluindo autenticação, endpoints, exemplos de solicitação/resposta e fluxos de trabalho passo a passo, consulte a [documentação da API de Migração de Decisão](decisioning-migration-api.md).
...
* [API de migração de decisão](decisioning-migration-api.md)
