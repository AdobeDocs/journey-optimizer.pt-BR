...
title: Notificações por push e Adobe Journey Otimizer
...
# Notificações por push e Adobe Journey Otimizer {#get-started-push}
...
1. O token de push gerado anteriormente é passado para a Adobe Experience Platform e sincronizado com o Perfil do cliente em tempo real; isso é feito no OOTB com um SDK de cliente fácil de integrar
...
1. As mensagens de push podem ser incluídas na tela de orquestração em Jornadas
1. Após a publicação da jornada, os perfis do cliente com base nas condições da jornada são qualificados para receber notificações por push, as cargas das mensagens por push são personalizadas nesta etapa
...
1. Os comentários dos serviços de mensagens são anotados, erros e sucessos são registrados para relatórios em relatórios do Journey Live &amp; Global
...
* **Provedores de serviços de notificação por push** são os principais serviços da Web de componentes que fornecem notificações de servidores remotos para aplicativos móveis.
   ...
   * [Serviço de notificação por push (APNs) da Apple](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/APNSOverview.html) - para enviar notificações para o aplicativo móvel iOS

* **SDK móvel da Adobe Experience Platform** que fornece APIs de integração do lado do cliente para dispositivos móveis por meio de SDKs compatíveis com Android e iOS. O SDK fornece um [!DNL Adobe Journey Optimizer] Exposição de uma variedade de APIs específicas para mensagens de push e habilitação do fluxo de dados, como registrar o token de push ou enviar eventos de rastreamento de push ou qualquer outro evento de experiência personalizada para a Adobe Experience Platform. O SDK também fornece uma variedade de outras extensões que permitem outros recursos da Adobe Experience Cloud e de parceiros de terceiros.

   A integração do SDK também requer a configuração da Adobe Experience Platform [Coleta de dados](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html){target=&quot;_blank&quot;} serviços como:

   * Criação de um armazenamento de dados para configurar os conjuntos de dados do perfil e do evento de experiência com base nos quais os dados fluem para a Adobe Experience Platform
...
* **[!DNL Adobe Journey Optimizer]** : depois que as integrações do aplicativo móvel com os componentes mencionados acima estiverem em vigor e os perfis do cliente na Adobe Experience Platform, você poderá criar e orquestrar notificações por push em [!DNL Adobe Journey Optimizer] para interagir com seus usuários.
