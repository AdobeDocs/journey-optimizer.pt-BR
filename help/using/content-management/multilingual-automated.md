...
**Nesta página:** saiba como criar conteúdo multilíngue com tradução automática, configurando um projeto de tradução e configurações de idioma e, em seguida, enviando seu conteúdo de campanha ou jornada a um provedor de tradução para revisão e publicação.
...
1. Comece a criar condições adicionando um atributo, evento ou público-alvo para definir seu grupo-alvo.
   ...
   1. Acesse o menu **[!UICONTROL Regras]** e crie uma nova regra. Informações detalhadas sobre como criar regras estão disponíveis na [!DNL Adobe Experience Platform] [Documentação da Coleção de Dados](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/ui/rules){target="_blank"}
