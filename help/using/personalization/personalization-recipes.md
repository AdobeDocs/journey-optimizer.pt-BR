...
Envolva a chave hifenizada em acentos graves dentro de uma expressão PQL: `{%= profile.events.\`order-total\` > 100 %}`. Backticks are not supported in plain Handlebars interpolation — use a `{% let %}` como uma etapa intermediária, se necessário.
