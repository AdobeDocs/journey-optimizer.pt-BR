...
1. Arraste e solte os campos desejados do painel esquerdo no espaço de trabalho central e configure-os de acordo com suas necessidades.
   ...
   * **Atributos**: perfis que vivem em Atlanta, São Francisco ou Seattle nascidos após 1980.
     ...
     >[!NOTE]
     >
     >O atributo `frequencyMap` não tem suporte para uso em definições de segmento e não pode ser usado como parte dos critérios de segmentação de público-alvo. Para direcionamento baseado em frequência, considere usar regras de limite de frequência em regras de negócios.
...
O público-alvo agora está pronto para uso em suas jornadas. Para saber mais, consulte [esta seção](../audience/about-audiences.md).
