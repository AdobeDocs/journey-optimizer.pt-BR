...
* Selecione **Marketing acionado por API** para campanhas baseadas em público-alvo
  ...
  Projetado para a comunicação baseada em segmentos ou públicos-alvo, na qual a mesma atualização é enviada para vários usuários. Por exemplo: pontuações esportivas, atualizações de eventos e experiências compartilhadas.

* Selecione **Transacional acionado por API** para campanhas individuais

  Usuários individuais pretendidos identificados por seus perfis. Por exemplo: status do pedido, rastreamento de entrega.
