...
Para evitar esses problemas, o [!DNL Journey Optimizer] cria uma versão de texto adicional de suas mensagens para que os cupons, intervalos de desconto, planos de ação e outras prioridades apareçam antecipadamente em uma cópia linear clara.

>[!NOTE]
>
>Essa versão de texto dedicada não é igual à versão de texto simples padrão ou personalizada de suas mensagens. [Saiba mais](text-version-email.md)
...
>
>O recurso Otimizar texto de email para caixas de entrada de IA gera apenas uma versão de texto dedicada no Journey Optimizer; não garante como um assistente de terceiros interpretará ou exibirá a mensagem. Leia mais sobre as [limitações e riscos da IA de caixa de entrada de terceiros](#inbox-ai-risks).
...
* **Controlando perguntas e respostas da caixa de entrada** — Quando você espera que os destinatários perguntem aos assistentes *sobre o que é o email* ou *quais são as ofertas*, uma versão forte da caixa de entrada de ia reduz os resumos parciais e evita depender de respostas fornecidas pela Web que não estejam vinculadas à sua cópia aprovada.
...
1. A versão gerada é exibida.

   ![Versão gerada otimizada para caixas de entrada de IA](assets/optimize-ai-inbox-output.png){zoomable="yes" width="80%"}

   >[!NOTE]
   >
   >**Otimizar texto de email para caixas de entrada de IA** não altera o design, o layout ou as imagens do HTML.

1. Para fazer alterações no conteúdo gerado automaticamente, selecione a opção **[!UICONTROL Habilitar edição]** e edite manualmente o conteúdo conforme necessário.

1. Quando estiver satisfeito com sua versão, clique no botão **[!UICONTROL Otimizar Email]** para confirmar.

1. Seu email agora foi otimizado com sucesso para caixas de entrada de IA.

1. Para acessar ou editar a versão otimizada, clique no botão **[!UICONTROL Otimizado para Caixa de Entrada de IA]**.
...
1. A versão otimizada é exibida. Você pode **[!UICONTROL Remover a otimização]** ou clicar em **[!UICONTROL Reotimizar]** para gerar uma nova versão.
   ...
   >[!NOTE]
   >
   >Se você fizer alterações no conteúdo original do HTML, precisará otimizar novamente a versão para as caixas de entrada da IA.
...
* **Exposição legal e de marca** — Resumos de IA incorretos ou incompletos ainda podem gerar confusão no cliente ou disputas sobre promoções, termos ou linguagem de recusa. **Otimizar email para caixas de entrada de IA** não garante que o modelo de terceiros reproduzirá fielmente a versão otimizada do seu email.
