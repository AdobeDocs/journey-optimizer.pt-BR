...
* **`inAudience()`tempo da condição:** Ao usar `inAudience()` em um nó de condição em uma jornada de Leitura de Público, a associação de segmento é lida a partir da projeção em lote do perfil. A atualização de dados nesta projeção carrega um SLT de até **2 horas** após a assimilação. Para obter detalhes completos sobre cenários de tempo de propagação, consulte a [documentação da função inAudience](functions/functioninaudience.md#propagation-timing).
