...
* Uma nova página está disponível para a Gestão de decisões. Ele lista todos os operadores, auxiliares e funções compatíveis ao criar expressões em conteúdo de oferta, regras de decisão e fórmulas de classificação. Use esta lista para evitar erros de tempo de execução. Somente as funções documentadas são compatíveis com a Gestão de decisões. [Leia mais](../offers/personalization-editor-supported-functions.md)
