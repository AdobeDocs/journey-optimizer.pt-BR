...
* Uma nova página de **Demonstração interativa** está disponível para os Desafios de fidelidade, com um link para uma demonstração autoguiada e clicável que abrange o fluxo de criação de desafios do profissional de marketing (incluindo Traga seus próprios dados e os painéis de insights), a experiência do cliente final e o Gerenciamento de Desafios de fidelidade no CX Coworker. [Leia mais](../loyalty-challenges/loyalty-challenges-demo.md)

* A página **Personalizar o fundo do email** foi expandida e aprimorada. Agora, ela documenta a lista suspensa completa **Posicionamento da imagem** para imagens de fundo e adiciona novas práticas recomendadas para cores e imagens de fundo, incluindo uma recomendação para testar imagens de fundo em clientes de email reais, em vez de depender exclusivamente da visualização do Designer de email. [Leia mais](../email/backgrounds.md)

* A página **Design de conteúdo do zero com o Designer de email** foi reorganizada e esclarecida: ela distingue a estrutura de coluna **[!UICONTROL n:n]** das estruturas de predefinição fixa, documenta que a contagem de colunas de uma estrutura pode ser aumentada sem perder conteúdo existente, explica o comportamento de empilhamento de colunas em dispositivos móveis e adiciona uma nova etapa no uso de **[!UICONTROL Módulos]** para iniciar rapidamente a criação de emails. [Leia mais](../email/content-from-scratch.md)

* A página **Projetar a jornada** agora inclui uma seção de tutorial completa sobre a nova experiência de tela, que aborda como adicionar atividades, usar os ícones da barra de ferramentas, selecionar várias atividades para ações em massa, copiar e colar atividades e unir ou separar ramificações. [Leia mais](../building-journeys/using-the-journey-designer.md#canvas-capabilities)

* Novas orientações foram adicionadas para verificar a entrega de ação personalizada: a página **Exemplos de consulta de conjunto de dados** agora explica como escolher entre os conjuntos de dados de Evento de feedback de mensagem, Rastreamento de email e Evento de etapa de jornada, dependendo do tipo de ação, e documenta como resolver um erro &quot;Tabela não provisionada para conjunto de dados&quot;. As páginas **Visão geral dos eventos de etapa da jornada** e **Solução de problemas de execução de jornada ativa** foram atualizadas de acordo, esclarecendo que uma chamada de ação personalizada bem-sucedida apenas confirma que o Journey Optimizer executou a ação, não que o sistema externo tenha entregue uma mensagem. [Leia mais](../data/datasets-query-examples.md#choose-the-correct-dataset)

* Foram adicionadas informações sobre o CX Coworker à página **Trabalhar com IA**, que abordam o que é o CX Coworker, como ele se relaciona com o Assistente de IA e referências à documentação oficial do Coworker. Páginas de habilidades dedicadas também foram adicionadas a cada guia de recursos — [Habilidades do CX Coworker para jornadas](../building-journeys/journeys-coworker-skills.md), [Habilidades do CX Coworker para fidelidade](../loyalty-challenges/loyalty-coworker-skills.md) e [Ferramentas de gerenciamento de conteúdo do CX Coworker](../content-management/content-management-coworker-skills.md). [Leia mais](../start/ai-features.md#cx-coworker)

* Uma nova habilidade **Analisar anomalias de jornada** foi documentada em **Analisar jornada** na página do CX Coworker. Ela detecta picos, quedas ou estagnações inesperadas nas contagens de entrada, saída ou envio de uma jornada em relação às linhas de base históricas e executa diagnósticos de somente leitura para revelar uma provável causa raiz. [Leia mais](../building-journeys/journeys-coworker-skills.md#journey-analyze)
...
* A página **Medidas de proteção e limitações** foi corrigida para refletir o fato de que fragmentos visuais com mais de 100 KB ou fragmentos de expressão com mais de 200 KB não podem mais causar problemas de truncamento na entrega de email: agora se aplica uma única medida de proteção de tamanho de fragmento de 700 KB. [Leia mais](../start/guardrails.md#fragments-guardrails)

* A página **Criar uma atividade em tempo real** foi corrigida: o campo `executionMetadata` está disponível somente para campanhas **transacionais acionadas por API**, não para campanhas de marketing acionadas por API, conforme declarado anteriormente. [Leia mais](../mobile-live/create-mobile-live.md#metadata)

* A documentação do **Conjunto de dados de evento de feedback de mensagem do AJO** foi expandida para esclarecer que abrange o feedback de entrega de mensagens em todos os canais (Email, SMS/RCS/MMS, Correspondência direta), não apenas email e push, e agora inclui uma seção **Classificar execuções de teste e não teste** explicando como interpretar o campo `isTestExecution`, incluindo `NULL` ou valores ausentes. [Leia mais](../data/datasets-query-examples.md#classify-test-executions)

* Um novo recurso **Gerenciamento de conteúdo** foi documentado para o CX Coworker, viabilizado por 15 ferramentas MCP de leitura/gravação que permitem descobrir, criar, atualizar, clonar e publicar modelos de conteúdo, fragmentos, páginas de destino e conteúdo de mensagem em linha de jornada/campanha usando prompts de linguagem natural. [Leia mais](../content-management/content-management-coworker-skills.md#content-management)

* A documentação **Adicionar conteúdo à página de destino** agora descreve uma opção **Tornar campo de formulário obrigatório** para caixas de seleção de consentimento: quando habilitada, o formulário não pode ser enviado, a menos que a caixa de seleção esteja marcada, e a verificação é aplicada no lado do cliente e no lado do servidor. [Leia mais](../landing-pages/lp-content.md#use-form-component)

* A página **Introdução à Simulação de jornada** foi atualizada para documentar que os nós de Decisão de conteúdo e o método de regra de direcionamento da atividade **Otimizar** agora são compatíveis com a Simulação (listados anteriormente como bloqueios), com uma nova tabela **Comportamento de decisão** que detalha como a elegibilidade da oferta, as regras de elegibilidade, os públicos-alvo e os métodos de classificação são avaliados durante uma execução da simulação. [Leia mais](../building-journeys/simulate-journey-gs.md#limitations)

* A página **Converter imagens em modelos de conteúdo de email** foi corrigida para remover um requisito de permissão impreciso: a permissão **Gerenciar modelos de conteúdo** não é necessária para acessar e criar modelos com o conversor de imagem para HTML — somente a permissão **Gerar conteúdo** é necessária. [Leia mais](../content-management/image-to-html.md#access-image-to-html)

* A página **Sistemas externos (ações personalizadas)** foi corrigida: o disjuntor de pontos de acesso de ação personalizada lenta agora é ativado quando mais de 20% das chamadas em uma janela de 120 segundos excedem **5 segundos** (documentado anteriormente como 10 segundos). [Leia mais](../configuration/external-systems.md#response-time)
...
* As páginas **Dados e conjuntos de dados de fidelidade** e **Introdução às fontes** foram atualizadas para incluir o LAVA como um conector de fidelidade e recompensas compatível, junto com Talon.One, Capillary e Kobie. [Leia mais](../loyalty-challenges/loyalty-data-and-datasets.md)

* As páginas **Delegar um subdomínio** e **Configurar subdomínios da Web** foram atualizadas para refletir que, dependendo do seu contrato de licença, agora é possível delegar até 3.000 subdomínios, a partir do limite anterior de 100. [Leia mais](../configuration/delegate-subdomain.md#guardrails)

## Agosto de 2026 {#august-2026}

* A página **Adicionar fragmentos visuais aos emails** agora esclarece que um fragmento com conteúdo dinâmico e um estado padrão vazio aparece em branco no Designer de email — simule com um perfil correspondente para visualizar o conteúdo. [Leia mais](../email/use-visual-fragments.md#fragment-dynamic-content)

* A página **Rastrear mensagens** foi atualizada para esclarecer que caracteres de URL incompatíveis (por exemplo, apóstrofos) devem ser codificados por porcentagem e que deixá-los sem codificação pode invalidar os links rastreados e os parâmetros de rastreamento de URL. [Leia mais](../email/message-tracking.md#insert-links)

* A página **Enviar usando ondas** foi atualizada para documentar que a última onda em uma jornada de público-alvo de leitura deve ser agendada dentro de **6 dias e 18 horas** a partir do início da jornada. Exceder essa janela aciona um erro de validação e impede que a jornada entre no modo de teste ou seja ativada. [Leia mais](../delivery/send-using-waves.md#limitations-guardrails)

* Uma nova seção **Suprimir eventos de feedback** foi adicionada à página **Coleta de dados para a gestão de decisões**, documentando como usar o sinalizador `dryRun` para suprimir eventos de decisão durante o teste e impedir que o feedback seja capturado para contadores de relatórios e de limite de frequência. [Leia mais](../offers/data-collection/data-collection.md#suppress-feedback)

* Uma nova página **Escolher um método de validação** está disponível. Ela compara Simulação de jornada, Modo de teste e Execução de teste de jornada — os dados que cada um usa, se envia mensagens reais, erros comuns a serem evitados e um guia de decisão para escolher o método certo em cada estágio da criação de uma jornada. [Leia mais](../building-journeys/choose-validation-method.md)
...
* A descrição de **Destinos** em **Navegação à esquerda > Seções principais** foi atualizada para indicar que organizações com [!DNL Real-Time CDP] ou [!DNL Adobe Journey Optimizer] também podem ativar públicos-alvo para destinos de personalização elegíveis, como [!DNL Adobe Target], do catálogo de destinos da Experience Platform. [Leia mais](../start/user-interface.md#main-sections)

* Vídeos tutoriais foram adicionados à documentação de Desafios de fidelidade para a criação de desafios, a configuração de provedores de recompensas e o monitoramento do desempenho de desafios. [Assista aos vídeos de desafio](../loyalty-challenges/create-challenges.md#video), [assista ao vídeo do provedor de recompensas](../loyalty-challenges/reward-definition-guide.md#video) e [assista ao vídeo de relatórios](../loyalty-challenges/loyalty-reporting.md#video).
...
* A documentação da **API de migração do serviço de decisão** foi aprimorada com orientações abrangentes sobre como migrar objetos da Gestão de decisões para o Serviço de decisão. As novas seções incluem: referência de mapeamento de entidade com 10 convenções de nomenclatura, cobertura dentro e fora do escopo, comparações detalhadas de modelos de solicitação/resposta, três padrões de implementação (lado do cliente, lado do servidor, híbrido) com processamento de cookies, requisitos de rastreamento de eventos com 5 exemplos JSON de evento, pré-requisitos de migração entre sandboxes, um processo de migração completo de 5 etapas e perguntas frequentes sobre migração. [Leia mais](../experience-decisioning/decisioning-migration-api.md)
