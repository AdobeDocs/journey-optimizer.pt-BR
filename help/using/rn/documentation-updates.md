...
* A interface das campanhas agora apresenta duas guias separadas: **Ação** e **API acionada**. A documentação foi atualizada adequadamente, com informações para cada tipo de campanha organizadas em seções dedicadas para melhorar a clareza e a usabilidade. [Leia mais](../campaigns/get-started-with-campaigns.md)

* As páginas [Introdução à delegação de subdomínio](../configuration/about-subdomain-delegation.md) e [Delegar um subdomínio](../configuration/delegate-subdomain.md) foram atualizadas para apresentar melhor os diferentes métodos de delegação e as etapas de configuração.

* Uma observação foi adicionada à seção Fragmentos, especificando que, quando o rastreamento é ativado em uma jornada ou campanha, se os links estiverem presentes em um fragmento e se esse fragmento for usado em uma mensagem, esses links serão rastreados como todos os outros links incluídos na mensagem. [Saiba mais](../content-management/create-fragments.md#content)

* As medidas de proteção e limitações aplicáveis à delegação de subdomínio no Journey Optimizer foram enriquecidas e consolidadas em uma seção dedicada. [Leia mais](../configuration/delegate-subdomain.md#guardrails)
...
* As grades de proteção aplicáveis aos fragmentos foram enriquecidas. [Leia mais](../start/guardrails.md#fragments-guardrails).

* Adição de uma observação para especificar que os links adicionados às mensagens expiram após 25 meses e os links para mirror pages após 90 dias. [Leia mais](../email/message-tracking.md)
