...
* Uma garantia sobre o direcionamento de perfis pseudônimos com canais de entrada foi adicionada às páginas de pré-requisitos da [web](../web/web-prerequisites.md#profile-management-guardrail), [no aplicativo](../in-app/inapp-configuration.md#profile-management-guardrail), [experiência baseada em código](../code-based/code-based-prerequisites.md#profile-management-guardrail) e [cartão de conteúdo](../content-card/content-card-configuration-prereq.md#profile-management-guardrail), bem como à página [Medidas de proteção e limitações](../start/guardrails.md). Essa garantia especifica que o direcionamento de perfis que não são autenticados ou conhecidos ainda aumenta sua contagem total de perfis ativáveis e pode afetar seu faturamento. É por isso que a Adobe recomenda configurar um TTL (Time-To-Live) para a exclusão automática de perfis, a fim de gerenciar a contagem de perfis ativáveis e os custos associados.
...
* Uma seção de solução de problemas foi adicionada à página de registro do DMARC para abordar a latência de propagação de DNS. [Leia mais](../configuration/dmarc-record.md#troubleshooting)
...
* Uma proteção sobre como direcionar perfis pseudônimos com canais de entrada foi adicionada à página Medidas de proteção e limitações: direcionar visitantes não autenticados aumenta a contagem total de perfis acionáveis, portanto, a Adobe recomenda definir um Tempo de vida (TTL) para a exclusão automática de perfis para gerenciar os custos associados. [Leia mais](../start/guardrails.md#profile-management-inbound)

* Dois tutoriais sobre como configurar o Web SDK para decisões e experiências baseadas em código agora são referenciados na página Amostras de métodos de implementação baseados em código. [Leia mais](../code-based/code-based-decisioning-implementations.md#tutorials)
