...
* Uma nova página de **esquema de Exportação de Mensagens do AJO** documenta cada campo no Conjunto de Dados de Exportação de Mensagens do AJO, com tipos de dados e hierarquia para o email e a carga de SMS exportados. [Leia mais](../configuration/message-export-schema.md)
...
* A documentação da **Atividade de bifurcação** foi aprimorada com uma nova seção **Exemplos** que ilustra como usar a atividade de bifurcação para dividir um público em duas ramificações de email paralelas — uma de Marketing e outra de Transação — em uma única execução de campanha. [Leia mais](../orchestrated/activities/fork.md#fork-examples)
...
* A documentação **Aproveitar fragmentos na documentação de políticas de decisão** agora inclui notas de garantia para o canal **Email**: **[!UICONTROL Simular conteúdo]** não exibe fragmentos de expressão do item de decisão, enquanto **[!UICONTROL Enviar prova]** e as campanhas ativadas exibem. A página também declara que **[!UICONTROL fragmentos visuais]** não podem ser atribuídos a um item de decisão — somente **fragmentos de expressão** são suportados neste contexto. [Leia mais](../experience-decisioning/fragments-decision-policies.md)

## Março de 2026 {#march-2026}

* A documentação para **visualizar experiências baseadas em código com o Experience Decisioning** agora esclarece que **[!UICONTROL Simular conteúdo]** é somente visualização de conteúdo. Os dados de contexto de solicitações Edge em tempo real não são simulados na pré-visualização de criação. [Leia mais](../code-based/test-code-based.md#preview-code-based)

* A documentação **Usar dados do Adobe Experience Platform** foi atualizada: as medidas de proteção não indicam mais que as pesquisas de conjunto de dados não podem ser encadeadas, refletindo o comportamento atual do produto. [Leia mais](../data/lookup-aep-data.md)

* A documentação de atividade **Atualizar Perfil** foi atualizada para documentar o suporte à atualização de até cinco atributos de perfil em uma única ação. [Leia mais](../building-journeys/update-profiles.md)

* A documentação da atividade **Ler público-alvo** e das **propriedades da Jornada** foi atualizada para esclarecer o ciclo de vida da jornada de 91 dias para jornadas recorrentes sempre ativas. A seção Programação agora confirma explicitamente que as jornadas recorrentes sem data final permanecem Ativas após 91 dias, e as Perguntas frequentes sobre tempo limite global foram expandidas para distinguir o perfil de 91 dias TTL da janela de relatório de 91 dias. [Leia mais](../building-journeys/read-audience.md#schedule)

* A documentação de atividade de **Pesquisa de conjunto de dados** foi atualizada para esclarecer que a chave de pesquisa deve ser configurada no modo avançado para que a sintaxe `@datasetLookup{}` funcione em atividades de condição downstream. Uma seção de solução de problemas foi adicionada com orientação sobre como resolver o erro &quot;Pesquisa de conjunto de dados não encontrada&quot;. [Leia mais](../building-journeys/dataset-lookup.md#troubleshooting)
...
* A documentação das **Campanhas orquestradas, medidas de proteção e limitações** e **Introdução aos conectores de Fontes** foi atualizada para esclarecer que, para o Change Data Capture baseado em arquivo, o campo `_change_request_type` é obrigatório e seus valores devem ser minúsculas `u` (substituição) ou `d` (exclusão), e não maiúsculas. [Leia mais](../orchestrated/guardrails.md)

* A documentação **Adicionar links e rastrear mensagens** foi atualizada com orientações sobre como os identificadores de rastreamento (urlID) são gerados: uma urlID exclusiva só é atribuída quando a URL e o rótulo são exclusivos. Para rastrear a mesma URL em vários emails (ou várias vezes em um email), os usuários devem usar um rótulo exclusivo para cada URL semelhante; caso contrário, [!DNL Journey Optimizer] não poderá determinar qual link foi clicado. [Leia mais](../email/message-tracking.md#track-across-multiple-emails)

* A documentação **Criar perfis de teste** foi atualizada com uma observação importante sobre os requisitos do descritor de identidade: quando um conjunto de dados é excluído e recriado, o esquema deve manter o descritor de identidade correto no campo de identidade principal. Sem ela, os perfis assimilados não serão sinalizados como `testProfile = true` mesmo que a assimilação seja concluída com êxito. Uma lista de verificação de solução de problemas foi adicionada. [Leia mais](../audience/creating-test-profiles.md)

* A documentação da atividade **Ler público-alvo** foi atualizada para esclarecer que uma atividade **de Evento Comercial** é uma exceção à regra de que Ler público-alvo deve ser a primeira atividade em uma jornada. Uma observação também foi adicionada referenciando a atividade **Otimizar** como uma alternativa avançada para controlar o direcionamento de público. [Leia mais](../building-journeys/read-audience.md)

* **Enviar usando ondas** no jornada agora está disponível. O sinalizador de Disponibilidade Limitada foi removido da documentação. [Leia mais](../building-journeys/send-using-waves.md)

* A documentação da atividade **Jump** foi aprimorada com uma nova seção de estratégia de design — **subjornadas de tamanho reduzido** — explicando como dividir fluxos completos complexos em subjornadas menores e focalizadas conectadas por meio da atividade Jump. [Leia mais](../building-journeys/jump.md#jump-strategy)

* A documentação de **Marcas** foi atualizada com orientações sobre o uso de categorias de marcas como uma alternativa às convenções de nomenclatura complexas. Uma nova seção explica como configurar categorias de tags para um gerenciamento de jornadas escalável. [Leia mais](../building-journeys/tags.md)
...
* A documentação da **fonte de dados do Adobe Experience Platform** foi atualizada com uma observação de que somente esquemas baseados em perfil individual XDM são suportados na configuração do Data Source. [Leia mais](../datasource/adobe-experience-platform-data-source.md)

* A documentação de **TTL (Time-to-live) dos conjuntos de dados** foi aprimorada com uma nova entrada de perguntas frequentes para identificar claramente quais conjuntos de dados estão sujeitos ao TTL. O TTL se aplica exclusivamente a conjuntos de dados de série temporal — conjuntos de dados do tipo registro, como conjuntos de dados de entidade, conjuntos de dados de classificação e repositórios de objetos de decisão, não estão sujeitos ao TTL e não serão afetados pela implantação da garantia. [Leia mais](../data/datasets-ttl.md)

* A documentação das **Propriedades de Jornada** e **Pausar uma jornada** foi atualizada para documentar os novos campos de pausa e retomada disponíveis nos detalhes técnicos da jornada. O botão **Copiar detalhes técnicos** agora inclui `lastPausedAt`, `lastPausedBy`, `lastPausedById`, `lastResumedAt`, `lastResumedBy` e `lastResumedById`, além do bloco `pausedJourneySettings` existente. Uma nova seção também foi adicionada à página **Pausar uma jornada** explicando como exibir os carimbos de data/hora de pausa e retomar diretamente das propriedades da jornada. [Leia mais](../building-journeys/journey-properties.md)

## Fevereiro de 2026 {#february-2026}

* Uma nova página está disponível para a Gestão de decisões. Ela lista todos os operadores, auxiliares e funções compatíveis ao personalizar o conteúdo da oferta (representações) com o editor de personalização. Use esta lista para evitar erros de tempo de execução. Somente as funções documentadas são compatíveis ao personalizar conteúdo no Offer Decisioning. [Leia mais](../offers/offer-library/personalization-editor-supported-functions.md)

* A documentação **Criar políticas de decisão** e **Usar políticas de decisão em mensagens** foi atualizada para Email: uma observação agora explica que quando a mesma oferta pode ser selecionada por mais de uma política de decisão no corpo do email, o mecanismo desduplica ofertas (cada posicionamento recebe uma oferta diferente). Para exibir a mesma oferta em vários posicionamentos (por exemplo, cabeçalho e rodapé), use **Reutilizar saída de decisão**. [Leia mais](../experience-decisioning/create-decision-policy.md)
...
* A documentação **Converter imagens em modelos de conteúdo** foi aprimorada com medidas de proteção e recomendações expandidas, casos de uso comuns e orientações mais claras para converter designs de imagem em modelos de conteúdo editáveis do HTML. Também menciona o fato de que agora você pode usar um tema como entrada para a conversão. [Leia mais](../content-management/image-to-html.md)
...
* Adição de uma observação na seção Atividade de condição da jornada para documentar que a avaliação da condição falhará para perfis que contenham mais de duas identidades entre dispositivos. [Leia mais](../building-journeys/conditions.md)
...
* A página de ações do canal integrado foi reorganizada. [Leia mais](../building-journeys/journey-action.md)
