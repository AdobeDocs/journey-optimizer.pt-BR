...
* A documentação dos identificadores complementares no jornada foi atualizada com os seguintes esclarecimentos:

   * Depois de adicionar um identificador complementar a um esquema, um novo evento (para jornadas acionadas por eventos) ou um novo grupo de campos (para jornadas de público-alvo de leitura) deve ser criado. As entidades existentes não são atualizadas automaticamente e não reconhecerão o novo identificador.

   * Os identificadores complementares não são validados em relação às políticas DULE (Label Usage Labeling &amp; Enforcement) e não são considerados durante as verificações de governança de dados no jornada.

[Saiba mais](../building-journeys/supplemental-identifier.md)

* A página Otimização em campanhas foi atualizada para refletir o fato de que a otimização agora também está disponível em jornadas. [Leia mais](../campaigns/campaigns-message-optimization.md)

* Um link para o vídeo tutorial descrevendo como aproveitar a otimização de mensagens em uma campanha foi adicionado. [Leia mais](../campaigns/campaigns-message-optimization.md)
