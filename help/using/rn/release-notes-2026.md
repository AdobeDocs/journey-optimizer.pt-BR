...
* **Redesign do fluxo de criação da Campanha de ação**: o fluxo de criação da Campanha de ação do Adobe Journey Optimizer foi reprojetado para fornecer uma experiência do usuário significativamente mais intuitiva, eficiente e contínua.
  ...
  Data de disponibilidade: 23 de agosto de 2026
...
O Journey Optimizer apresenta os desafios de fidelidade, um novo recurso nesta versão.
...
Os recursos e melhorias a seguir foram adicionados nesta versão.
...
<tr>
<td>
<p>Agora você pode configurar uma jornada ou ação de campanha para incluir vários canais de saída (Email, Push, SMS) e permitir que o Journey Optimizer entregue automaticamente por meio do melhor canal para cada cliente. Três modos de otimização estão disponíveis:</p>
<ul>
<li>Classificação manual: especifique a ordem de canal de sua preferência.</li>
<li>Preferência do cliente: use o canal de preferência do cliente em seu perfil (atributo Consentimentos e preferências do Experience Data Model).</li>
...
* **Canal do WhatsApp: suporte a modelos de fluxo do WhatsApp**: agora você pode enviar modelos de fluxo do WhatsApp no Adobe Journey Optimizer para fornecer experiências interativas em várias telas, como pesquisas e captura de leads. As respostas são capturadas no envio e armazenadas como conteúdos JSON brutos no novo conjunto de dados de evento de rastreamento de canal do Journey Optimizer:
...
* **Integrações aprimoradas do provedor personalizado – Dispositivos móveis**: as integrações personalizadas do provedor agora oferecem maior flexibilidade com mensagens principais e atualizações de cabeçalho:

  * Personalização de cabeçalho: agora é possível editar o valor padrão do cabeçalho Tipo de conteúdo e adicionar até 10 parâmetros de cabeçalho personalizados.

  * Suporte a conteúdo SMS: adição de suporte para funções auxiliares do Adobe Journey Optimizer no conteúdo SMS, incluindo encode64.

### Administração {#july-26-administration}

As seguintes funcionalidades e melhorias foram adicionadas à administração e ao gerenciamento de dados nesta versão.

<table>
<thead>
<tr>
<th><strong>Lista de permissão de IPs do Firewall de aplicativos web</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>O Adobe Journey Optimizer agora oferece suporte à lista de permissões de IP do Firewall de aplicativos web para páginas de destino, permitindo que as organizações imponham que todas as solicitações recebidas sejam roteadas exclusivamente por meio de sua infraestrutura configurada do Firewall de aplicativos web. Com esse aprimoramento, os clientes podem configurar o Journey Optimizer para rejeitar qualquer solicitação direta que ignore a camada do Firewall de aplicativos web, garantindo que as políticas de segurança definidas em ferramentas como o Imperva sejam aplicadas de forma consistente.</p>
<p>Esse recurso fortalece a postura de segurança para empresas com requisitos rigorosos de acesso à rede, dando a elas controle total sobre o fluxo de tráfego para suas páginas de destino hospedadas no Journey Optimizer.</p>
<p><img src="assets/do-not-localize/allowed-ips.gif"></p>
<p>Para obter mais informações, consulte a <a href="../configuration/waf-ip-allowlist.md">documentação detalhada</a>.</p>
...
* **Gerenciar domínios para personalização completa/básica do URL**: agora é possível criar e gerenciar domínios aprovados para personalização completa e básica do URL diretamente das configurações de Administração no Adobe Journey Optimizer, sem precisar entrar em contato com o suporte da Adobe. [Saiba mais](../email/url-personalization.md#personalize-complete-base-url)

  Data de disponibilidade: 30 de julho de 2026

* **Medidas de proteção de TTL (Tempo de vida) do conjunto de dados — sandboxes existentes**: as medidas de proteção de TTL (tempo de vida) para conjuntos de dados gerados pelo sistema do Journey Optimizer (90 dias no repositório de perfis, 13 meses no data lake) será aplicada em **sandboxes e organizações de clientes já existentes** a partir de **1º de outubro de 2026**. [Saiba mais](../data/datasets-ttl.md#ttl-guardrail)

### Design de email {#july-26-email}

Os seguintes recursos e melhorias foram adicionados ao design de email nesta versão.
...
<p>Para obter mais informações, consulte a <a href="../email/email-modules.md">documentação detalhada</a>.</p>
...
<thead>
<tr>
<th><strong>Nova interface</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Uma <b>nova interface</b> foi introduzida para a tela de jornada, oferecendo melhor desempenho para jornadas grandes, layout automático para legibilidade aprimorada e uma experiência de criação guiada.</p>
<p><img src="../building-journeys/assets/journey-new-canvas.png"></p>
<p>Para acessar a nova interface, clique em <b>Nova experiência</b>. Essa configuração é salva no nível da jornada, para que a jornada reabra na nova experiência por padrão. Para reverter, clique em <b>Experiência antiga</b>. <a href="../building-journeys/using-the-journey-designer.md#canvas-capabilities">Saiba mais</a>.</p>
...
* [!BADGE Descontinuação]{type=Negative} **Públicos-alvo em lote não são mais compatíveis com o nó de Qualificação de público-alvo e os Critérios de saída** - a partir de setembro de 2026, o Journey Optimizer bloqueará a publicação de qualquer jornada usando um público-alvo em lote em um nó de Qualificação de público-alvo ou nos critérios de saída. Um aviso de validação já foi exibido na tela de jornada.  As jornadas ativas existentes não são afetadas. As jornadas novas, de rascunho e duplicadas que incluem essa configuração devem ser atualizadas antes de setembro de 2026. Use um público-alvo de transmissão no nó de Qualificação de público-alvo ou alterne para uma atividade de Público-alvo de leitura. Para Critérios de saída, use um público-alvo de transmissão. [Saiba como migrar suas jornadas](../building-journeys/aq-batch-audiences-migration.md)

* **Públicos-alvo externos na Simulação de jornada** - a Simulação de jornada agora oferece suporte a Públicos-alvo externos. Ao simular jornadas direcionadas a públicos-alvo de CSV ou Composição de público-alvo federado, é possível simular atributos de enriquecimento desses públicos-alvo diretamente pelo formulário da interface ou por uma importação de JSON. A interface exibe dinamicamente apenas os atributos específicos de enriquecimento usados na lógica de jornada, permitindo a validação precisa das ramificações de decisão e regras de personalização antes da ativação. [Saiba mais](../building-journeys/simulate-journey.md)
...
<thead>
<tr>
<th><strong>Direcionamento baseado em arquivo em Campanhas orquestradas</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>As campanhas orquestradas agora permitem carregar um <strong>arquivo CSV ou TXT</strong> diretamente na tela da campanha como o público-alvo, sem primeiro ingerir o arquivo na Adobe Experience Platform. Os dados do arquivo são consumidos no momento da execução e não são mantidos como um conjunto de dados da Adobe Experience Platform. Durante a configuração do arquivo, é possível definir mapeamentos de coluna, tipos de dados, tratamento de NULL e políticas de erro por coluna. As linhas que falham na validação são rejeitadas e registradas antes da execução da campanha, mantendo o público-alvo limpo sem pré-processamento manual. Isso é particularmente adequado para envios ad hoc ou campanhas de lista de parceiros em que a criação de um pipeline de ingestão completo não é prática.</p>
...
* **Permissão Exibir transições de Campanha orquestrada** – adição de uma nova permissão **Exibir transições de Campanha orquestrada** para substituir a opção herdada **Exibir arquivo em Campanhas orquestradas**. Essa alteração permite ocultar os resultados da visualização nas transições de campanha para oferecer suporte à conformidade com informações de identificação pessoal.
...
* **Atributos personalizados dinâmicos de itens de decisão** - os atributos personalizados de item de decisão agora podem ser personalizados no momento da entrega usando dados de perfil, contextuais e de público-alvo. Isso elimina a necessidade de manter ofertas duplicadas para pequenas variações de conteúdo, permitindo que os profissionais de marketing gerenciem menos itens de decisão mais flexíveis. [Saiba mais](../experience-decisioning/items.md#attributes)
...
* **Novas funções auxiliares nas expressões de personalização** - novas funções auxiliares agora estão disponíveis nas expressões de personalização:

  * `appendQueryParams`: anexa um parâmetro de consulta a um URL ou o substitui se a chave já existir.
  * `dateBetween`: verifica se uma data está dentro de um intervalo de datas inicial e final (inclusive).
  * `equalsAnyIgnoreCase`: retorna verdadeiro quando uma string corresponde a qualquer valor fornecido, ignorando maiúsculas e minúsculas.
  * `getUrlFragment`: extrai a parte do fragmento de um URL (a parte após #).
  * `join`: concatena elementos de matriz em uma única string usando um separador.
  * `decode64`: decodifica uma string codificada em Base64. Se a entrada não for Base64 válido, a string de entrada original será retornada inalterada.
  * `parseJson`: analisa uma string JSON em uma variável estruturada que pode ser usada no modelo.
  ...
  * `ampSubstr`: retorna uma parte de uma string entre os índices de início e término especificados.
...
* **“Assistente de IA” renomeado para “Gerar conteúdo”**: o Assistente de IA foi renomeado para Gerar conteúdo no Adobe Journey Optimizer. Esta atualização está limitada à nomenclatura e terminologia; nenhuma alteração funcional foi introduzida. Rótulos de navegação, botões, menus e caixas de diálogo para geração de conteúdo, geração de imagem, expressões de personalização e experimentação de conteúdo foram renomeados de “Assistente de IA” para “Gerar conteúdo”.

  Data de disponibilidade: 30 de julho de 2026

* **Melhorias multilíngues** - as configurações de idioma agora podem ser duplicadas a partir de uma configuração ativa existente, portanto, você não precisa mais reconstruir totalmente uma configuração para fazer alterações. Também é possível copiar uma condição de uma localidade para outra enquanto se cria as Configurações de idioma, simplificando as configurações de sites com vários idiomas.

  Data de disponibilidade: 30 de julho de 2026

### Conteúdo e integrações {#july-26-integration}

As seguintes melhorias estão chegando ao gerenciamento de conteúdo e às integrações nesta versão.

<table>
<thead>
<tr>
<th><strong>Temporizador de contagem regressiva usando o Dynamic Media</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p><strong>A integração do Journey Optimizer com o Adobe Experience Manager Dynamic Media</strong> habilita a personalização no momento da abertura para modelos do Dynamic Media, desbloqueando casos de uso hiperpersonalizados. Os clientes podem criar e publicar modelos personalizados no Adobe Experience Manager e usá-los no Journey Optimizer, com dados renderizados no momento da abertura.</p>
<p>Para obter mais informações, consulte a <a href="../integrations/aem-dynamic.md#countdown">documentação detalhada</a>.</p>
<p> Data de disponibilidade: 30 de julho de 2026</p>
</td>
</tr>
</tbody>
...
* **Novas ferramentas do servidor MCP do AJO** – o servidor MCP do [!DNL Adobe Journey Optimizer] agora expõe cinco **ferramentas de configuração de canais** adicionais de somente leitura, o que permite consultar configurações de canais, recursos de suporte e ações de marketing diretamente do assistente de IA. Agora você pode usar as opções **Configurações de canal de lista** (em todos os canais do AJO), **Obter configuração de canais**, **Recursos de configuração de lista**, **Obter recurso de configuração** e **Ações de marketing de lista**. [Leia mais](../integrations/ajo-mcp.md#mcp-tools)
...
* **Novas métricas estimadas de cliques para relatórios de email**: para oferecer uma visão mais precisa do engajamento real dos clientes, novas métricas estimadas já estão disponíveis nos relatórios de Jornadas, Campanhas e Canais ativos.

  * CTR (Taxa de click-through) estimado: calculado como os cliques estimados em relação ao número total de mensagens entregues.

  * CTOR estimado (taxa de clique para abrir): calculado como cliques estimados em relação ao número total de aberturas estimadas.
...
* **Atalhos de inicialização rápida no inventário Modelos**: o botão **[!UICONTROL Mais ações]** da lista Modelos de conteúdo agora fornece acesso rápido a ações comuns: editar detalhes do modelo, simular conteúdo e excluir um modelo. Atalhos específicos para canais adicionais também estão disponíveis: para modelos de email, edite o corpo do email, exiba ou envie uma prova, execute um relatório de spam e renderize o email; para modelos SMS, verifique a contagem de caracteres e o número de segmentos. [Saiba mais](../content-management/access-content-templates.md#edit)
...
* [!BADGE Descontinuação]{type=Negative} **Públicos-alvo em lote não são mais aceitos no nó de Qualificação de público-alvo e nos Critérios de saída**: a partir de setembro de 2026, o Journey Optimizer bloqueará a publicação de qualquer jornada usando um público-alvo em lote em um nó de Qualificação de público-alvo ou nos Critérios de saída. As jornadas ativas existentes não são afetadas. As jornadas novas, de rascunho e duplicadas que incluem essa configuração devem ser atualizadas antes de setembro de 2026. Use um público-alvo de transmissão no nó de Qualificação de público-alvo ou alterne para uma atividade de Público-alvo de leitura. Para Critérios de saída, use um público-alvo de transmissão. [Saiba como migrar suas jornadas](../building-journeys/aq-batch-audiences-migration.md)

* **Interromper uma jornada pausada diretamente**: agora você pode interromper uma jornada diretamente do estado **Pausado**. Anteriormente, uma jornada pausada precisava ser retomada para **Ativa** antes de poder ser interrompida. [Leia mais](../building-journeys/journey-pause.md#stop-close-paused)
...
* **Limite de jornadas ativas aumentado e novas medidas de proteção**: agora é possível ter até **200 jornadas ativas**, um aumento em relação ao limite anterior de 100. [Leia mais](../start/guardrails.md#journeys-guardrails-journeys)

  Data de disponibilidade: 18 de junho de 2026. Essa funcionalidade será implementada gradualmente em todas as regiões nos próximos dias.

### Campanhas orquestradas {#june-26-oc}

Os recursos e melhorias a seguir estão chegando às campanhas orquestradas nesta versão.

* **Personalização baseada em loops para dados relacionais**: o editor de personalização agora oferece suporte a um bloco de loop que itera sobre coleções relacionais — como pedidos, contas ou reservas — e renderiza um bloco de conteúdo por registro dentro de um único email ou SMS. As coleções são configuradas por meio do seletor de dados usando tokens de personalização, sem a necessidade de escrita de expressão. [Leia mais](../orchestrated/add-personalization.md#enrichment-collections)
...
* **Usar fragmentos de conteúdo do Adobe Experience Manager na Decisão**: agora você pode mapear fragmentos de conteúdo do Adobe Experience Manager para itens na Decisão e utilizá-los em políticas de decisão para entregar o fragmento certo ao cliente correto, no momento oportuno. Anteriormente lançado em disponibilidade limitada, este recurso já está disponível para todos os ambientes (disponibilidade geral). [Leia mais](../experience-decisioning/fragments-decision-policies.md)
...
* **Novas métricas estimadas de cliques para relatórios de email**: para oferecer uma visão mais precisa do engajamento real dos clientes, novas métricas estimadas já estão disponíveis nos relatórios de Jornadas, Campanhas e Canais.

  * **CTR estimado** (taxa de click-through): calculado como cliques estimados em relação ao número total de mensagens entregues.
...
* **Relatório de cliques estimados para SMS**: uma nova métrica de cliques estimados agora está disponível em relatórios de jornadas, campanhas e canais de email e SMS. Essa métrica exclui o tráfego identificado de bots e de interação não humana (NHI) para fornecer uma visão mais clara do engajamento genuíno do cliente. A métrica Cliques existente permanece disponível e continua a relatar o total de cliques.

### Melhorias de usabilidade {#june-26-usability}

* **Pastas para jornadas**: agora você pode organizar suas jornadas em **pastas** para melhorar a navegação e o gerenciamento na interface. [Leia mais](../building-journeys/journey-ui.md#journeys-folders)
...
<thead>
<tr>
<th><strong>Fragmentos de jornada (disponibilidade limitada)</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td>
<p>As campanhas orquestradas agora podem ser vinculadas acionando uma campanha orquestrada diretamente da <strong>atividade Fim</strong> de outra campanha orquestrada.</p>
<p>Isso permite dividir a lógica de orquestração complexa em fluxos menores e reutilizáveis que podem ser chamados de várias campanhas principais, em vez de serem reconstruídos a cada vez. O conteúdo transmitido no tempo de execução está disponível para segmentação e personalização na campanha downstream para que cada campanha vinculada possa se comportar com base no contexto que recebe.</p>
...
* **Adicionar links na atividade de Enriquecimento**: agora a funcionalidade Adicionar link está disponível na atividade de Enriquecimento para campanhas orquestradas. Isso permite criar uma relação direta entre os dados da tabela de trabalho e as tabelas existentes do banco de dados.
...
<thead>
<tr>
<th><strong>Otimização das regras de decisão e da fórmula de classificação usando IA</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>[!DNL Adobe Journey Optimizer] agora usa IA para detectar regras de decisão e fórmulas de classificação que podem ser simplificadas. No inventário, aparece um indicador vermelho nas regras para as quais a IA identificou uma oportunidade de otimização. Clicar no indicador exibe a expressão original junto com a versão sugerida pela IA. A partir daí, você pode baixar um arquivo para revisar como os perfis simulados são avaliados por cada versão, confirmar se eles se comportam de forma idêntica e, em seguida, substituir a expressão pela otimizada.</p>
...
<p>Observe que, embora a opção Deeplink esteja disponível para todos os clientes, os deep links só funcionarão se as etapas de configuração e implementação de aplicativos móveis necessárias tiverem sido concluídas.</p>
...
* **Restringir a quebra de herança nos fragmentos** - ao criar ou editar um fragmento, agora é possível escolher se ele pode ser modificado quando usado em emails. Bloquear um fragmento garante que ele permaneça sincronizado em todos os lugares onde aparece, evitando edições locais que poderiam violar os padrões da marca ou os requisitos de conformidade. Essa configuração pode ser atualizada posteriormente, aplicando-se a usos futuros. [Leia mais](../content-management/create-fragments.md#lock-visual-fragment)

  Data de disponibilidade: 21 de maio de 2026

### Mensagens por dispositivo móvel (SMS, MMS e RCS) {#may-26-mobile}

Os seguintes recursos e melhorias foram adicionados às mensagens por dispositivo móvel nesta versão.

<table>
<thead>
<tr>
<th><strong>Novo canal de mensagens por dispositivo móvel e mensagens RCS aprimoradas</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>SMS, MMS e RCS agora estão unificados em uma ação de <strong>Mensagem por dispositivo móvel</strong> no Adobe Journey Optimizer, facilitando o gerenciamento de todos os tipos de mensagens por dispositivo móvel em um único local. Como parte dessa atualização, agora é possível criar mensagens RCS de mídia avançada, incluindo imagens, carrosséis e ações sugeridas, diretamente no Journey Optimizer por meio de uma nova experiência de criação nativa.</p>
...
* **Deep links no conteúdo SMS** - agora é possível adicionar deep links ao conteúdo SMS usando a função auxiliar de URL. Isso garante que os destinatários sejam levados diretamente ao conteúdo no aplicativo desejado, sem serem redirecionados por meio de um navegador da web ou uma loja de aplicativos, desde que você tenha concluído as etapas de configuração e implementação de aplicativos móveis necessárias. [Leia mais](../email/deeplinks.md)

### Canal do WhatsApp {#may-26-whatsapp}

As seguintes melhorias foram adicionadas ao canal do WhatsApp nesta versão.

* **Compatibilidade e rastreamento de botões do WhatsApp**: os modelos do WhatsApp agora são compatíveis com **Resposta rápida**, **Chamada para ação — URL** e **Chamada para ação — telefone**. **Copiar código** não é compatível. O Journey Optimizer envia botões compatíveis e rastreia interações, além dos outros relatórios de canal que você já tem.

* **Dados de contexto do canal do WhatsApp**: agora o Journey Optimizer captura dados adicionais de interação retornados do canal do WhatsApp e os armazena no **Conjunto de dados EmailTrackingExperienceEvent do AJO** no grupo de campos `whatsAppChannelContext`. [Leia mais](../whatsapp/send-whatsapp.md#whatsapp-channel-context)

  +++ Os campos a seguir são capturados e podem ser usados para construir públicos-alvo e analisar o engajamento no WhatsApp

  * **`messageType`** – tipo de mensagem do WhatsApp (ex: `templateBased`, `response`)
  * **`inboundMessage`** – conteúdo de resposta recebida (por exemplo, `stop`, `start`, `subscribe`)
  * **`inboundNumber`** – ID do remetente onde a mensagem foi recebida
  * **`channelType`** – categoria de canal (`Utility`, `Marketing` ou `Promotional`)
  * **`profileNumber`** – número de telefone do qual a mensagem foi recebida
  * **`origTimestamp`** – Carimbo de data e hora original da Meta/WhatsApp
  * **`status`** – status de entrega incluindo feedback padronizado do provedor (`sent`, `delivered`, `bounce`, `error`, `delay`, `duplicate`, `denylist`, `exclude` ou `unknown`) e a mensagem de status bruta do provedor
...
<thead>
<tr>
<th><strong>Seletor do Assessor de conteúdo</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Agora o Journey Optimizer usa o <strong>seletor do Assessor de conteúdo</strong>, um modal unificado para selecionar fragmentos de conteúdo e ativos do Experience Manager. O novo seletor inclui:</p>
<ul>
<li><strong>Navegar, pesquisar e filtrar </strong> em todos os ativos e fragmentos.</li>
<li><strong>Pesquisa semântica de IA</strong>: descreva o que você procura em linguagem simples, por exemplo, “café nas montanhas”, para exibir ativos contextualmente relevantes com base no significado e no conteúdo, não apenas em correspondências de texto. Consultas multilíngues também são compatíveis.</li>
...
* **Acesso ao repositório entre organizações no Seletor de ativos**: agora é fácil selecionar ativos de repositórios de várias organizações diretamente no Seletor de ativos do Adobe Experience Manager.
...
* **Classificação e redimensionamento de coluna**: agora as listas **Campanhas**, **Fragmentos** e **Modelos** permitem classificar quando clicamos em qualquer cabeçalho de coluna. Na exibição das pastas de Campanhas, também estão disponíveis a classificação e filtragem por **[!UICONTROL Prioridade]** e **[!UICONTROL Configuração de canal]**. As larguras de coluna nas listas **Fragmentos** e **Modelos** também são redimensionáveis — arraste a borda da coluna para ajustar os dados mais importantes para você. [Saiba mais](../start/search-filter-categorize.md#filter-lists)
...
* **Dica de ferramenta de URL do link no editor do canal de push**: quando um URL em qualquer campo de link ou mídia é muito longo para ser exibido, um ícone de dica de ferramenta fica sempre visível ao lado do campo. Passe o mouse sobre ele para ver o URL completo. [Saiba mais](../push/design-push.md#on-click-behavior)
...
## Notas da versão de abril de 2026 {#april-26-rn}
...
<tr>
<td>
<p>Agora o Adobe Journey Optimizer fornece um <strong>servidor MCP (Model Context Protocol)</strong> que revela operações de campanha, configuração de canais e sandbox dentro de qualquer aplicativo compatível com MCP de forma direta. Com essa integração, diferentes personalidades podem colaborar em torno dos mesmos dados de orquestração. Em vez de escrever consultas na API REST do Adobe Journey Optimizer ou navegar por várias telas de interface, você pode descrever sua intenção em conversação e permitir que o LLM chame as ferramentas de MCP apropriadas. Esse recurso está disponível atualmente no Claude Web e Desktop.</p>
...
<tr>
<td>
<p>[!DNL Adobe Journey Optimizer] agora inclui o <strong>Assistente de IA</strong> diretamente no editor de personalização e o Designer de email que converte prompts de linguagem natural em expressões de personalização e lógica condicional válidas. Não é necessária experiência em sintaxe. Descreva a personalização que deseja realizar e a IA gera um código pronto para uso que pode ser aplicado imediatamente ou refinado por meio de prompts de acompanhamento.</p>
...
* **Tamanho do conteúdo da jornada atual visível nas propriedades da jornada**: agora o painel de propriedades da jornada exibe o tamanho atual do conteúdo da jornada em comparação ao limite configurado. Por exemplo, *1,5 MB (de 4 MB)*. Esse indicador somente leitura ajuda a monitorar a complexidade da jornada antes da publicação e a evitar erros causados por exceder o limite de tamanho do conteúdo. [Leia mais](../building-journeys/journey-properties.md#journey-payload-size)
...
<tr>
<td>
<p>Os parâmetros de URL em links de rastreamento e de página de destino adicionados às suas mensagens de email agora podem ser criptografados, fornecendo uma camada adicional de segurança para dados de parâmetros confidenciais.</p>
<ul>
...
<tr>
<td>
<p>Com [!DNL Journey Optimizer], você pode capturar atributos de perfil por meio das páginas de destino.</p>
...
<p>Anteriormente disponibilizada em versão limitada para clientes nos Estados Unidos e na Austrália, essa funcionalidade agora está disponível para todos os ambientes (disponibilidade geral).</p>
...
<tr>
<td>
<p>Uma nova atividade <strong>Test</strong> está disponível em Campanhas orquestradas. Essa atividade direciona a execução do fluxo de trabalho para diferentes ramificações com base em condições definidas, permitindo validar a lógica e as configurações da campanha antes de ativar as entregas ativas.</p>
...
<thead>
<tr>
<th><strong>Suporte à pesquisa de conjunto de dados em jornadas</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Uma nova atividade de <strong>Pesquisa de conjunto de dados</strong> em jornadas permite recuperar dados dinamicamente de conjuntos de dados de registros da Adobe Experience Platform em tempo de execução — dando acesso a informações que não fazem parte do perfil ou do conteúdo do evento, para que as interações com o cliente permaneçam relevantes e oportunas.</p>
<p>Anteriormente lançada em disponibilidade limitada para um conjunto restrito de organizações, a atividade de pesquisa de conjunto de dados em jornadas agora está disponível para todos os clientes com direito à [dataset lookup](../data/lookup-aep-data.md), embora permaneça em disponibilidade limitada.</p>
...
<thead>
<tr>
<th><strong>A atividade Ação substitui atividades de jornada específicas do canal</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Após a Disponibilidade geral da <strong>Atividade de ação</strong> em fevereiro de 2026, as atividades de canal nativas legadas (Email, Push, SMS, No aplicativo, Web, Experiência baseada em código e Cartão de conteúdo) na tela da jornada foram descontinuadas.</p>
<p>Agora você deve usar a atividade única Ação para configurar todas as ações de canal, substituindo a necessidade de nós específicos do canal separados.</p>
<p>As jornadas existentes que usam atividades de canal legadas continuam funcionando sem nenhuma alteração ou migração necessária.</p>
...
<thead>
<tr>
<th><strong>Editor avançado de HTML para modelos de email</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>O modo HTML avançado para modelos de conteúdo de email permite editar a origem do HTML do conteúdo no Designer de email, adicionar expressões avançadas (como condições) na origem e alternar entre a exibição do HTML e a exibição do Desktop sem perder as alterações.</p>
...
<tr>
<td>
<p>Permita a integração perfeita de modelos do Firefly padrão e personalizados, juntamente com modelos de imagem de terceiros aprovados, para oferecer maior flexibilidade, controle e alinhamento com a marca ao gerar imagens.</p>
<p>Escolha o modelo certo para suas necessidades:</p>
<ul><li> <strong>Modelo da Adobe</strong> (desenvolvido pelo Firefly Image Model 4) para geração imediata de imagens sem configuração adicional</li><li> <strong>Modelo de parceiro</strong> (desenvolvido pelo Gemini 2.5 Flash) para recursos especializados</li><li><strong>Modelos personalizados</strong> (modelos específicos da marca treinados em seus próprios ativos) para geração de itens consistentes com a marca que se alinham precisamente à identidade, ao estilo e às diretrizes visuais da marca.</li></ul>
...
<thead>
<tr>
<th><strong>Atividade em tempo real no iOS</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Leve experiências em tempo real diretamente para as Telas de bloqueio e o Dynamic Island dos clientes com a <strong>Atividade em tempo real no iOS</strong> no Adobe Journey Optimizer. Forneça atualizações em tempo real, desde o rastreamento de pedidos e o status de voos até contagens regressivas para eventos, pontuações ao vivo e progresso de entregas, sem exigir que os usuários abram seu aplicativo. Mantenha o público-alvo informado e engajado no momento exato, onde quer que ele esteja.</p>
<p>Anteriormente lançado na versão beta, este recurso já está disponível para todos os ambientes (disponibilidade geral).</p>
...
<thead>
<tr>
<th><strong>Monitoramento de modelo de IA</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>O Journey Optimizer agora permite monitorar a integridade, o status do treinamento e o desempenho dos modelos de IA de decisão. Isso permite verificar o sucesso do treinamento, solucionar problemas e entender o impacto nos resultados, a fim de selecionar as melhores ofertas para cada cliente que usa IA. Observe que esse recurso está disponível somente para <strong>Decisão</strong> (não para modelos legados da Gestão de decisões).</p>
<p>Este recurso está disponível atualmente apenas para modelos de <strong>otimização personalizada</strong> (não de otimização automática).</p>
...
<tr>
<td>
<p>As campanhas orquestradas agora podem ser acionadas por meio de um <strong>sinal de API</strong>. Para configurar isso, defina a campanha de destino como <strong>Acionada por um sinal</strong>, publique-a e, em seguida, dispare-a usando uma chamada de API. Todos os parâmetros incluídos na chamada de API estão disponíveis como variáveis na campanha em execução. Observe que as campanhas orquestradas acionadas por sinal continuam sendo campanhas <strong>em lote</strong> e são diferentes das campanhas acionadas por API.</p>
...
<tr>
<td>
<p>Em Campanhas orquestradas, agora é possível definir uma atividade de canal para a categoria <strong>Transacional</strong>. Essa categoria aplica configurações de canal transacional a essa atividade e é útil quando as regras de negócios não devem ser aplicadas ou quando a aceitação do cliente não é necessária.</p>
...
<p>Essa funcionalidade será implementada gradualmente em todas as regiões nos próximos dias.</p>
</td>
</tr>
</tbody>
</table>
...
* **Personalização completa/básica de URL**: você pode personalizar URLs de destino usando atributos de perfil (por exemplo, para o domínio ou caminho). Para habilitar esse recurso, forneça à Adobe sua lista de domínios aceitos. [Leia mais](../personalization/personalization-build-expressions.md#where)
...
* **Renovação de certificados de domínio do AJO mal sucedida**: agora você pode assinar o recebimento de alertas do sistema (por email ou na central de notificações do Journey Optimizer) quando um certificado de domínio usado para entrega de email estiver perto de expirar ou já tiver expirado. [Leia mais](../reports/alerts.md#alert-certificates-renewal-unsuccessful)
...
* **Conjunto de dados do evento de feedback de destinatário secundário do AJO renomeado**: o conjunto de dados `AJO Email BCC Feedback Event` foi renomeado para Conjunto de dados `AJO Secondary Recipient Feedback Event`. O impacto varia dependendo da sua situação:
  ...
  * **Usuários existentes**: somente o nome de exibição é atualizado. O nome da tabela subjacente permanece inalterado.
  * **Novos usuários e sandboxes**: o nome de exibição e o nome da tabela refletem o novo nome.
  * **Usuários existentes com novas sandboxes**: o nome de exibição e o nome da tabela são atualizados com o novo nome.
...
* **Detalhes de pausa e retomada nos detalhes técnicos da jornada**. Os **detalhes técnicos** da jornada agora incluem informações adicionais de pausa e retomada: a data e a hora da última pausa e retomada, o nome de exibição e o identificador interno do usuário que executou cada ação e um conjunto completo de configurações de jornada pausadas, como comportamento de pausa, duração máxima da pausa e estado de retomada automática. [Saiba mais](../building-journeys/journey-properties.md)
...
<tr>
<td>
<p>O Journey Optimizer oferece suporte a uma nova atividade genérica <strong>Ação</strong> que permite configurar ações individuais e grupos de ações de entrada com várias ações, possibilitando uma configuração simplificada de ações na tela da jornada. Em especial, este novo recurso permite:</p>
<ul>
...
<p><strong>Observação:</strong> todos os canais nativos agora estão acessíveis por meio da atividade de jornada de Ação. As atividades legadas do canal nativo serão descontinuadas na versão de março. As jornadas existentes que incluem ações legadas continuarão funcionando normalmente, nenhuma migração é necessária.</p>
</td>
</tr>
</tbody>
</table>
...
<thead>
<tr>
<th><strong>Envio em ondas de mensagens de saída</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Agora você pode agendar o envio de mensagens de campanhas ou jornadas do Journey Optimizer em lotes controlados ao longo do tempo.</p>
<p>O envio em ondas oferece os seguintes benefícios:</p>
<ul>
<li>Melhor capacidade de entrega: distribua os envios ao longo do tempo para ajudar a manter uma boa reputação do remetente e reduzir o risco de ser sinalizado como spam.</li>
<li>Controle de carga: evite sobrecarregar os sistemas subsequentes (por exemplo, centrais de atendimento, páginas de destino) limitando a quantidade de mensagens enviadas simultaneamente.</li>
<li>Casos de uso de alto volume e urgentes: adequados a grandes públicos-alvo ou quando é necessário controlar o tempo (por exemplo, capacidade da central de atendimento, aumento ou ofertas com limite de tempo).</li>
</ul>
...
<p>Em <strong>jornadas</strong>, esse recurso só está disponível para algumas organizações (Disponibilidade limitada). Para obter acesso, entre em contato com um representante da Adobe. Para obter mais informações, consulte a <a href="../building-journeys/send-using-waves.md">documentação detalhada</a>.</p>
...
<tr>
<td>
<p>Agora estão disponíveis APIs de ferramentas de migração para migrar programaticamente entidades de <strong>Gestão de decisões</strong> para <strong>Tomada de decisões</strong>, com os seguintes recursos:</p>
<ul>
...
<tr>
<td>
<p>Obtenha mais insights sobre a integridade e o desempenho de pontos de acesso de ação personalizados com um novo painel de monitoramento e dados de eventos da etapa da jornada aprimorados. Monitore chamadas bem-sucedidas, erros, taxa de transferência, tempos de resposta e tempos de espera na fila para entender rapidamente quando, onde e por que ocorrem anomalias.</p>
<p>Anteriormente lançado em disponibilidade limitada para uso em jornadas, este recurso já está disponível para todos os ambientes (disponibilidade geral).</p>
...
<thead>
<tr>
<th><strong>Apoio à decisão no canal de SMS</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Agora você pode personalizar e otimizar o conteúdo das mensagens de SMS com a Decisão. Use Pontuações de prioridade, Fórmulas ou Modelos de IA para exibir o melhor conteúdo para os clientes.</p>
...
* **Uso do evento de experiência em expressões de jornada**: desde 1º de abril de 2026, o uso de atributos de evento de experiência em expressões de jornada não é mais aceito em organizações que não usaram esse recurso nos últimos 90 dias. Esse recurso já está indisponível para novas organizações de clientes desde 8 de julho de 2025. Para obter alternativas, consulte [Pesquisa de evento de experiência em jornadas](../building-journeys/exp-event-lookup.md).
...
* **Usar temas para converter imagens em modelos de email**: ao converter uma imagem em um modelo de email no Journey Optimizer, agora é possível usar um tema como entrada para que o HTML gerado siga os parâmetros da sua marca. Estilos como cor do fundo, cor do botão, fontes, espaçamento entre linhas, margens e preenchimento são aplicados automaticamente, reduzindo o trabalho manual de design e fornecendo um modelo pronto para uso com edições mínimas. [Leia mais](../content-management/image-to-html.md)
...
* **Recuo de texto**: agora é possível aplicar recuo à esquerda personalizável à primeira linha de parágrafos em componentes de texto diretamente no painel de propriedades. <!--The new **Indentation** control lets you define indentation in pixels or percentage via a numeric input or slider, with live preview on the canvas. -->Isso melhora a legibilidade de conteúdo longo, como editoriais e artigos. [Leia mais](../email/get-started-email-style.md)
...
* **Webhooks de SMS**: os webhooks agora são aceitos por todos os provedores de SMS. Você pode configurar cada webhook de acordo com sua finalidade: webhooks de entrada para capturar mensagens recebidas e webhooks de feedback para receber confirmações de entrega, atualizações de status e outros eventos relacionados a mensagens. [Leia mais](../mobile/mobile-webhook.md)
...
<thead>
<tr>
<th><strong>Apoio à decisão no canal de push</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>Agora você pode personalizar e otimizar o conteúdo de <strong>Notificações por push</strong> com a <strong>Decisão</strong>. Use Pontuações de prioridade, Fórmulas ou Modelos de IA para exibir o melhor conteúdo para os clientes.</p>
<p>A Escolha de experiências com notificações por push exige uma versão específica do SDK móvel. Antes de implementar este recurso, verifique as <a href="https://developer.adobe.com/client-sdks/home/release-notes" target="_blank">notas de versão</a> para identificar a versão necessária e se você atualizou de acordo. Você também pode visualizar todas as versões do SDK disponíveis para sua plataforma <a href="https://developer.adobe.com/client-sdks/home/current-sdk-versions" target="_blank">nesta seção</a>.</p>
<p>Para obter mais informações, consulte a <a href="../experience-decisioning/create-decision.md">documentação detalhada</a>.</p>
...
<p>Os registros são retidos no conjunto de dados de exportação de mensagens do AJO por sete dias corridos a partir da ingestão. Durante esse período de retenção, você pode exportá-los para seu próprio armazenamento por meio de destinos da Experience Platform. O recurso é habilitado no nível de configuração de canais, fornecendo a você <strong>controle granular</strong> sobre quais mensagens são exportadas.</p>
...
* **Agendar campanha usando o fuso horário do perfil**: o agendamento de campanha agora pode usar o <strong>fuso horário</strong> de cada perfil para entregar mensagens no horário local desejado. [Leia mais](../campaigns/campaign-schedule.md)
  ...
  **Observação**: essa melhoria estará disponível apenas para um conjunto de organizações (Disponibilidade limitada).
