...
Os qualificadores de coleção também podem ser usados para agrupar ofertas em coleções. Para obter mais informações, consulte o tutorial em [criação de coleções](../../../offer-library/creating-collections.md).

Você pode exibir uma lista de todos os qualificadores de coleção em um contêiner executando uma única solicitação de GET para a [!DNL Offer Library] API.
