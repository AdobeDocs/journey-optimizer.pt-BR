...
Você pode pesquisar coleções específicas fazendo uma solicitação GET para a API [!DNL Offer Library] que inclui a coleção `@id` ou o nome da coleção no caminho da solicitação.
...
Uma resposta bem-sucedida retorna os detalhes do posicionamento, incluindo informações sobre ID de contêiner, ID de instância e coleção exclusiva `@id`.
