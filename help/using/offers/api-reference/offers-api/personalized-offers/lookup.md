...
Você pode pesquisar ofertas personalizadas específicas fazendo uma solicitação GET para a [!DNL Offer Library] API que inclui a ID de oferta personalizada no caminho da solicitação.
...
| `{ID}` | A ID da entidade que você deseja pesquisar. | `personalizedOffer1234` |
...
Uma resposta bem-sucedida retorna os detalhes da oferta personalizada, incluindo informações sobre sua oferta personalizada exclusiva `id`.
