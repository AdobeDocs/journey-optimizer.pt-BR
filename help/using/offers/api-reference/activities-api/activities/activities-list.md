...
Uma decisão (anteriormente conhecida como atividade de oferta) contém a lógica que informa a seleção de uma oferta.

É possível exibir uma lista de todas as decisões em um contêiner ao executar uma única solicitação de GET para a API [!DNL Offer Library].
