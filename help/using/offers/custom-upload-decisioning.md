...
Com o [!DNL Journey Optimizer] você pode aproveitar os dados de públicos-alvo criados com o Upload personalizado (arquivo CSV) no [!DNL Adobe Experience Platform]. Esses dados são compatíveis com os workflows de Gestão de decisões. Isso é particularmente útil quando os dados não são necessários no perfil, mas ainda são essenciais para fins de decisão.
...
* Use Públicos-alvo de upload personalizados no nível de decisão.
  ...
  ![Upload personalizado de público no nível de decisão](assets/csv-decision.png)
...
Na Gestão de decisões, somente determinadas funções são suportadas ao personalizar o conteúdo — consulte [Funções suportadas no editor de personalização](offer-library/personalization-editor-supported-functions.md).
