...
➡️ [Assista a este vídeo para saber mais sobre os fundamentos de capacidade de entrega de aquecimento de IP](#video)
...
| Configurar SPF e DKIM | Confirma que o servidor de envio é legítimo e autorizado | Manuseado automaticamente pela Adobe após a delegação de subdomínio e a criação de configuração de canal. [Saiba mais](delegate-subdomain.md) |
| Configurar o registro DMARC | Permite a emissão de relatórios de autenticação de email e políticas de imposição futuras | Manuseado automaticamente pela Adobe após a delegação de subdomínio e a criação de configuração de canal. [Saiba mais](dmarc-record.md) |
...
| Criar público-alvo de alto engajamento da Fase 1 | Aumenta as métricas de engajamento antecipado com os recipients mais ativos | Crie um público-alvo com menos de 5.000 contatos que abriram ou clicaram nos últimos 30 dias |
...
|------|----------|-----------------|------------------------|
| 1-3 | 0,5% | Seus destinatários mais envolvidos | Usar formato de texto simples e curto com uma call-to-action clara acima da dobra |
...
| 15-21 | 25% | Assinantes ativos mais levemente inativos | Usar conteúdo de marketing normal enquanto monitora as taxas de reclamação de perto |
| 22-28 | 50-100% | Lista completa (respeitando as listas de supressão) | Transição para sua cadência de envio de estado estável |

>[!NOTE]
>
>A Adobe Journey Optimizer fornece um [recurso de planos de aquecimento de IP](ip-warmup-gs.md) dedicado que automatiza o gerenciamento de volumes e simplifica o processo de aquecimento sem exigir configurações complexas do jornada.

## Uso do recurso de planos de aquecimento de IP da AJO {#ajo-warmup-feature}

O Adobe Journey Optimizer inclui um recurso simplificado de planos de aquecimento de IP que elimina a necessidade de limitação manual de volume por meio de configurações complexas de jornada. Essa funcionalidade garante uma abordagem padronizada para criar a reputação do remetente.
...
* **Relatórios ao vivo**: acesse a medição e a visualização em tempo real de suas campanhas na guia **[!UICONTROL Últimas 24 horas]**. [Saiba mais](../reports/live-report.md)

* **Integração com o Customer Journey Analytics**: para obter insights mais profundos, aproveite o Customer Journey Analytics para analisar dados do Adobe Experience Platform e criar visualizações personalizadas. [Saiba mais](../reports/report-gs-cja.md)

### Métricas do Target

Monitore esses indicadores-chave de desempenho durante o aquecimento:

| Métrica | Limite do público alvo | Ação se excedida |
|--------|-----------------|-------------------|
| Taxa de reclamação | ≤ 0,1 % | Segmento de auditoria e supressão de reclamantes crônicos |
| Taxa de rejeição permanente | ≤ 2 % | Revisar a qualidade da lista e as práticas de higiene |
| Taxa de abertura | ≥ 10% | Verifique se você está direcionando públicos envolvidos |

>[!TIP]
>
>Para obter uma análise abrangente da campanha, use os recursos do [relatório ao vivo da campanha](../reports/campaign-live-report.md#email-live) e do [relatório do Customer Journey Analytics](../reports/campaign-global-report-cja-email.md).
...
* **Manter a consistência**: mantenha os aumentos diários de volume abaixo de 30% semana a semana para preservar a reputação estabelecida
...
* **Decisões orientadas por dados**: controle diariamente as taxas de reclamação, rejeição e envolvimento e ajuste sua estratégia antes que os ISPs o penalizem

* **Autenticação primeiro, volume segundo**: resolva todos os problemas de SPF, DKIM e DMARC antes de começar a aumentar o volume

* **Questões de consistência**: os provedores de caixa de correio favorecem padrões de envio previsíveis; evitam picos de volume repentinos ou agendas de envio irregulares
