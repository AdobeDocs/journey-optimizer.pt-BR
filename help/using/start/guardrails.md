...
* Os fragmentos de expressão não estão disponíveis para os canais da web e no aplicativo.
...
* Uma instância do jornada para um perfil tem um tamanho máximo de 1 MB. Todos os dados coletados como parte da execução da jornada são armazenados nessa instância do jornada. Portanto, dados de um evento de entrada, informações de perfil recuperadas do Adobe Experience Platform, respostas de ação personalizadas etc. são armazenados nessa instância do jornada e afetam o tamanho da jornada. É aconselhável, quando uma jornada inicia com um evento, limitar o tamanho máximo desse payload do evento (por exemplo: abaixo de 800 KB) para evitar atingir esse limite após algumas atividades, na execução da jornada. Quando esse limite é atingido, o perfil está com status de erro e será excluído da jornada.
